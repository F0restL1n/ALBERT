...
def load_local_glue_data(task_path):
    ...
    # 明确列名
    column_names = ["index", "label", "unused", "sentence"]

    # 加载数据
    train_data = pd.read_csv(train_file, sep="\t", names=column_names, header=None, on_bad_lines="skip")
    dev_data = pd.read_csv(dev_file, sep="\t", names=column_names, header=None, on_bad_lines="skip")
