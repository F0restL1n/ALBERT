...
def load_local_glue_data(task_path):
    ...
    # 加载数据，跳过标题行
    train_data = pd.read_csv(train_file, sep="\t", header=0, on_bad_lines="skip")
    dev_data = pd.read_csv(dev_file, sep="\t", header=0, on_bad_lines="skip")
    ...
    # 删除空值行
    train_data = train_data.dropna(subset=["sentence1", "sentence2", "label"])
    dev_data = dev_data.dropna(subset=["sentence1", "sentence2", "label"])
